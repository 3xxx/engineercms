...
// 纯文件夹检索，作废，保留
func (c *MainController) Photo_back() {
...
// 触底追加数据
func (c *MainController) GetMonthPhotoData() {
	var yearmonth string
	// limit := "5"
	limit := c.GetString("limit")
	if limit == "" {
...
// 图片数据写入数据库
func (c *MainController) CreatePhotoData() {
	cwd, _ := os.Getwd()
	// logs.Info(cwd)
	photopath, err := web.AppConfig.String("photopath")
	if err != nil {
...
// 纯文件夹检索，作废
func (c *MainController) PhotoDetail_back() {
