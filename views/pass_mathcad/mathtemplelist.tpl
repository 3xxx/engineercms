...
<body>
  <div class="col-lg-12">
    ...
    window.operateEvents = {
      ...
      'click .pdf': function(e, value, row, index) {
        // alert("计算书模板预览功能待完善~");
        var url = '/v1/mathcad/getmathtemplepdf/'+row.ID
        window.open(url, "_blank", "")
      },
    ...
    function setArticle(value, row, index) {
      ...
      if (value.ID) {
        if (value.ID > 0) {
          articleUrl = '<a href="/v1/mathcad/matharticle/' + value.ID + '" title="查看" target="_blank"><i class="fa fa-file-text-o"></i></a>';
          return articleUrl;
        }
      }
    }
  ...
  function save2() {
    ...
    if (prodname) {
      $.ajax({
        type: "post",
        url: "/v1/mathcad/addmatharticle/"+projectid,
        ...
        success: function(data, status) {
          if (data.info=="ERROR"){
            ...
            $('#table0').bootstrapTable('refresh', { url: '/v1/mathcad/getmath/gettemples/{{.Id}}' });
          }
        },
      });
    } else {
