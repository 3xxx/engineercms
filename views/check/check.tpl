...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $.ajax({
      ...
      success:function(data,status){
        // console.log(data)
        $(".option").remove();
        $.each(data.processing,function(i,d){
  ...
  $('#Date').datepicker().on('hide', function(e) {
    ...
    // $.ajax({
    //   type: "GET",
    //   url: "/v1/checkin/monthcheck?activityId=3&year=2019&month=05",
    //   contentType: "application/json;charset=utf-8",
    //   dataType: "json",
    //   json: 'callback',
    //   success: function (json) {
  ...
  //   url : "/v1/checkin/monthcheck?activityId=", // 获取数据的地址
  //   queryParams:function queryParams(params) {   //设置查询参数
  //     var param = {
  //         limit: params.pageSize,   //每页多少条数据
  ...
  function getColumns() {
    // 加载动态表格
    $.ajax({
      url : '/v1/checkin/monthcheck?activityId=3&year=2019&month=04',
      type : 'get',
      dataType : "json",
      async : false,
