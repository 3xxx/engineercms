...
<head>
  ...
  <title> {{substr .ExcelTemple.TitleB 0 6}}...</title>
...
<body>
  <div class="col-lg-12">
    <h5>输入参数表（模板名称：{{.ExcelTemple.TitleB}} ；作者：{{.ExcelTemple.User.Nickname}} ；是否有效：{{.ExcelTemple.Status}}）</h5>
  ...
  <div class="col-lg-6">
    <button type="button" data-name="Submit1" id="Submit1" class="btn btn-danger" onclick="oncalculateExcel()"> <i class="fa fa-calculator"> 计算</i></button>
  ...
  $('#table0').bootstrapTable({
    ...
    url: '/v1/excel/getexcelcalinput/{{.ID}}',
    // striped: "true",
    columns: [{
      title: '序号',
  ...
  $('#table2').bootstrapTable({
    ...
    url: '/v1/excel/getexcelcaloutput/{{.ID}}',
    // striped: "true",
    columns: [{
        title: '序号',
        formatter: function(value, row, index) {
      ...
      formatter: onMsoNumberFormat
        ...
        editable: {
          ...
          type: 'text',
          pk: 1,
          url: '/v1/excel/putexcelcaloutput',
          title: 'Enter ExcelComment'
        },
      }
    ]
  ...
  $(function() {
    ...
    var ws = new WebSocket('ws://' + window.location.host + '/v1/excel/wsexcelcal/{{.ID }}');
  ...
  function oncalculateExcel() {
    //两种设置disabled属性
    $('#Submit1').attr("disabled", true);
    $("#pdflink").css("display", "none");
    // console.log(intDiff);
    timer(intDiff); //计时器开始
    var ws = new WebSocket('ws://' + window.location.host + '/v1/excel/postexcel/{{.ID }}');
    ws.onmessage = function(event) {
      var msg = JSON.parse(event.data);
      var ele = document.getElementById("content");
      ele.value = ele.value + msg.message + '\n';
      ...
      if (msg.info == "ERROR") {
        ...
        // $('#table2').bootstrapTable('append',msg.data)

        if (msg.pdflink!=""){
        $("#pdflink").css("display", "block");
        $("#pdflink").attr("href", "/v1/wx/getexcelpdf/"+msg.pdflink);
        }  
        $('#Submit1').attr("disabled", false);
        clearInterval(time),
        function() {
  ...
  function oncalculateExcel_back() {
    //两种设置disabled属性
    $('#Submit1').attr("disabled", true);
    $("#pdflink").css("display", "none");
    ...
    $.ajax({
      type: "post",
      url: "/v1/excel/postexcel",
      data: { excelid: {{.ID }}, description: description, inputdata: data2 },
      success: function(data, status) {
        if (data.info == "ERROR") {
          ...
          $("#pdflink").attr("href", "/v1/wx/getexcelpdf/"+data.pdflink);
