...
<body>
  <div class="col-lg-12">
    ...
    $(function() {
      ...
      $("#table0").bootstrapTable({
        url: '/v1/excel/getexcel/getexcels/{{.Id}}',
        method: 'get',
        search: 'true',
    ...
    function setArticle(value, row, index) {
      ...
      if (value.ID) {
        if (value.ID > 0) {
          articleUrl = '<a href="/v1/excel/excelarticle/' + value.ID + '" title="查看" target="_blank"><i class="fa fa-file-text-o"></i></a>';
          return articleUrl;
        }
      }
    }
    ...
    function addButton() {
      ...
      var url = '/v1/excel/uploadexcel/' + {{.Id }}
      window.open(url, "_blank", "")
    }
  ...
  function save2() {
    ...
    if (prodname) {
      $.ajax({
        type: "post",
        url: "/v1/excel/addexcelarticle/"+projectid,
        ...
        success: function(data, status) {
          if (data.info=="ERROR"){
            ...
            $('#table0').bootstrapTable('refresh', { url: '/v1/excel/getexcel/getexcels/{{.Id}}' });
          }
        },
      });
    } else {
