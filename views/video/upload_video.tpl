...
$("#input-ke-2").fileinput({
  language: 'zh',
  theme: "explorer",
  // uploadAsync: true,//异步上传，多文件每个都发一次请求
  //异步上传，参数为MultipartFile;会发送多个post请求，每个请求会进入一次后台的入口，后台参数需设置为MultipartFile
  ...
  uploadAsync: false, //同步上传，多文件只发一次请求
  uploadUrl: "/v1/wx/uploadvideodata/{{.Id}}",
  minFileCount: 1,
  maxFileCount: 20,
  maxFileSize: 200000,
  removeFromPreviewOnError: true,
  overwriteInitial: false,
  allowedFileExtensions: ["mp4","avi","mpg","mkv","mov","mp4","3gp","webm","wmv"], //接收的文件后缀
...
}).on("filebatchselected", function(event, data) { //同步批量上传，只请求一次post
