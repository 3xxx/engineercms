...
func CreateBusiness(business Business) (id uint, err error) {
	...
	// 查询项目名称和时间段
	//判断是否有重名
	result := db.Where("projecttitle = ? AND start_date = ? AND end_date = ? AND project_id", business.Projecttitle, business.StartDate, business.EndDate, business.ProjectID).First(&business)
