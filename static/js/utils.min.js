...
var Hutils = {
    ...
    getFrist:function (value) {
        var tmp = value.split("__hzwy23__");
        if (tmp.length==2){
            return tmp[0];
        }
        return value;
                ...
                $.HAjaxRequest({
                    ...
                    success: function(data){
                        ...
                        $("#h-main-content").find("div.active").each(function(index,element){
                            if (data_id == $(element).attr("data-id")){
                                if (__DEFAULT.newIframe == true || __DEFAULT.newIframe == "true") {
                                    $(element).find("iframe:eq(0)").attr("srcdoc",data);
                                } else {
                                    $(element).html(data);
                                }
                                return false;
    ...
    go_entry :function (e){
        ...
        $.HAjaxRequest({
            ...
            dataType:'text',
            success:function(d){
                var obj = document.getElementById("bigdata-platform-subsystem");
                obj.innerHTML = "";
                obj.style.display = "none";
                $("#bigdata-platform-subsystem").html(d).fadeIn();
            },
            error:function () {
                $.Notify({
                    title:"温馨提示：",
    ...
    initMenu:function(TypeId,Id,Group1,Group2,Group3){
        ...
        var __genDiv = function (element) {
            var res_id = element.Res_id;
            var res_class = element.Res_class;
            var res_bg_color =element.Res_bg_color;
            var res_img = element.Res_img;
            var res_name = element.Res_name;
            var res_url = element.Res_url;
            var open_type = element.Res_open_type;
            var newIframe = element.New_iframe;
